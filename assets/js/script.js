...
$(function () {
  ...
  if (gallery.length) {
    ...
    if (count && count > 0) {
      for (var n = count; n > 0; n--) {
        gallery.append('<img src="../assets/img/photography/' + category + '/' + (useThumbnails ? 'thumbnails/' : '') + n
          + (useThumbnails ? '_tn' : '') + '.jpg" data-image="../assets/img/photography/' + category + '/' + n + '.jpg" alt="">');
      }
  ...
  if (prints$.length && printsTags$.length && config.prints) {
    for (var category in config.prints) {
      ...
      config.prints[category].forEach(p => {
        ...
        $frame.find('img').attr('src', '../assets/img/photography' + p).one('load', function () {
          var $img = $(this);
  ...
  $('.likely .vkontakte,.likely .twitter').attr('data-title', document.title);
});
